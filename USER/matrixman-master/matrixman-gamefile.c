...
void displayPixel(uint8_t x, uint8_t y, char color) {
    ...
    drawPixel(x,y-2,Color888(colors[color][0],colors[color][1],colors[color][2]));
}
...
void displayGameOver(void) {
    setScrollOffsetFromEdge(13);
    setScrollMode(wrapForward);
    scrollText("Game Over", 1);
    while(getScrollStatus() > 0) { }
}

void displayClear(u8 Color){
  ClearBuff(0,1023);
}
